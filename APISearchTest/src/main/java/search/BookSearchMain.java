...
public class BookSearchMain {
	 public static void main(String[] args) {
	        ...
	        String apiURL = "https://openapi.naver.com/v1/search/book?query=" + text + "&sort=date";    // json 결과
	        URL url;
			try {
				url = new URL(apiURL);
				...
				for(int i=0; i<arr.length(); i++) {
					...
					downloagImage(arr.getJSONObject(i).getString("image"),str[1]);
				}
	    ...
	    // 웹에 있는 이미지 다운로드
	    public static void downloagImage(String url, String title) {
	    	try {
				URL imgUrl = new URL(url);
				URLConnection con = imgUrl.openConnection();	// 이미지 파일과 연결
				
				InputStream is = con.getInputStream();
				FileOutputStream fos = new FileOutputStream(title+".jpg");
				
				// is에서 받은 내용을 fos를 통해서 출력 -> stream으로 read/writer --> byte
